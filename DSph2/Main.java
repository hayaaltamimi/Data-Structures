package DSph2;
import java.util.InputMismatchException;
import java.time.LocalDate;
import java.time.format.DateTimeFormatter;
import java.util.Scanner;
import java.time.format.DateTimeParseException;

public class Main {
	public static Scanner input = new Scanner(System.in);

	//Products:
	static Products pm = new Products();

	//Orders:	
	static Orders order = new Orders();

	//Customer:
	public static Customers cdata = new Customers();

	// Reviews
	public static ReviewsManager rm=new ReviewsManager(pm, cdata);
	public static LinkedList<Review> reviews;

	// =====================================================================================================================
	public static boolean loadData() {
		try {
			System.out.println("---LOADING SYSTEM DATA---");
			//products:
			pm.loadProducts("products.csv");
			System.out.println("-----------------------------------");

			//customers:
			cdata.loadCustomers("customers.csv");
			System.out.println("-----------------------------------");

			// order:
			order.loadOrders("orders.csv");
			System.out.println("-----------------------------------");

			// reviews
			rm.loadReviews("reviews.csv");
			reviews = rm.getReviews();
			if (reviews.size() == 0)
				return false;
			System.out.println("-----------------------------------");
			linkOrdersToCustomers();
			System.out.println("All data loaded successfully!");
			return true;

		} catch (Exception e) {
			System.out.println("Error loading data: " + e.getMessage());
			return false;
		}
	}

	// =====================================================================================================================
	public static int mainMenu() {
		System.out.println("\n**** MAIN MENU ****");
		System.out.println("--------------------------");
		System.out.println("1. Products Menu");
		System.out.println("2. Customers Menu");
		System.out.println("3. Orders Menu");
		System.out.println("4. Reviews Menu");
		System.out.println("5. Exist");
		System.out.println("Enter your choice: ");
		return input.nextInt();
	}

	// =====================================================================================================================
	public static void productsMenu() {
		int choice = 0;
		do {
			System.out.println("\n**** PRODUCTS MENU *****");
			System.out.println("1. Display All Products");
			System.out.println("2. Search Product by ID");
			System.out.println("3. Display Products by Price Range");
			System.out.println("4. Track Out of Stock Products");//not in this phase
			System.out.println("5. Show Top 3 Rated Products");
			System.out.println("6. Show Top 3 Most Reviewed Products");
			System.out.println("7. Add New Product");
			System.out.println("8. Remove Product");//not
			System.out.println("9. Update Product");
			System.out.println("10. Common Highly Rated Products Between 2 Customers");//not 
			System.out.println("11. Display all customers who reviewed a product");
			System.out.println("12. Return to Main Menu");
			System.out.print("Enter your choice: ");

			try {
				choice = input.nextInt();

				switch (choice) {
				case 1:
					pm.displayAllProducts();
					break;

				case 2:
					System.out.print("Enter Product ID to search: ");
					int productId = input.nextInt();
					Product foundById = pm.searchById(productId);
					if (foundById != null) {
						System.out.println("\nProduct Found:");
						System.out.printf("%-12s %-25s %-10s %-8s %-8s%n",
								"ProductID", "Name", "Price", "Stock", "AvgRating");
						System.out.println("------------------------------------------------------------------------------------------------");
						foundById.display();
						System.out.println("------------------------------------------------------------------------------------------------");
					} 
					else {
						System.out.println("Product with ID " + productId + " not found");
					}
					break;

				case 3:
					boolean validRange = false;
					double minPrice = 0, maxPrice = 0;

					while (!validRange) {
						System.out.print("Enter minimum price: ");
						minPrice = input.nextDouble();
						System.out.print("Enter maximum price: ");
						maxPrice = input.nextDouble();

						if (minPrice > maxPrice) {
							System.out.println("Error: Minimum price cannot be greater than maximum price!");
							System.out.println("Please enter a valid price range.");
						} else {
							validRange = true;
						}
					}
					pm.displayProductsByPriceRange(minPrice, maxPrice);
					break;

				case 4:
					System.out.println("\n--- OUT OF STOCK PRODUCTS ---");
					pm.trackOutOfStock();
					break;

				case 5:
					pm.top3ByRating();
					break;

				case 6:
					pm.top3ByMostReviewed();
					break;

				case 7: 
					System.out.println("\n--- ADD NEW PRODUCT ---"); 
					System.out.print("Enter Product ID: "); 
					int newId = input.nextInt(); 
					input.nextLine();  
					System.out.print("Enter Product Name: "); 
					String newName = input.nextLine(); 
	
				    System.out.print("Enter Price: ");
				    double newPrice = input.nextDouble();
				    if (newPrice<0) {//validating input
				        System.out.println("Price cannot be negative!");
				        break;               
				    }

				    System.out.print("Enter Stock Quantity: ");
				    int newStock = input.nextInt();
				    if (newStock < 0) {//validating input
				        System.out.println("Stock cannot be negative!");
				        break;
				    }

					Product newProduct = new Product(newId, newName, newPrice, newStock); 
					pm.addProduct(newProduct); 
					break;

				case 8:
					System.out.print("Enter Product ID to remove: ");
					int removeId = input.nextInt();
					pm.removeProduct(removeId);
					break;

				case 9:
					System.out.print("Enter Product ID to update: ");
					int updateId = input.nextInt();
					Product existing = pm.searchById(updateId);
					if (existing != null) {
						input.nextLine(); 
						System.out.print("Enter New Name: ");
						String updatedName = input.nextLine();
						System.out.print("Enter New Price: ");
						double updatedPrice = input.nextDouble();
						System.out.print("Enter New Stock: ");
						int updatedStock = input.nextInt();

						Product updatedProduct = new Product(updateId, updatedName, updatedPrice, updatedStock);
						pm.updateProduct(updateId, updatedProduct);
					} else {
						System.out.println("Product not found.");
					}
					break;

				case 10:
					System.out.print("Enter First Customer ID: ");
					int custId1 = input.nextInt();
					System.out.print("Enter Second Customer ID: ");
					int custId2 = input.nextInt();

					// Validate that both customers exist
					Customer customer1 = cdata.searchById(custId1);
					Customer customer2 = cdata.searchById(custId2);

					if (customer1 == null && customer2 == null) {
						System.out.println("Error: Both customer IDs " + custId1 + " and " + custId2 + " do not exist!");
					} else if (customer1 == null) {
						System.out.println("Error: Customer ID " + custId1 + " does not exist!");
					} else if (customer2 == null) {
						System.out.println("Error: Customer ID " + custId2 + " does not exist!");
					} else {
						// Both customers exist, proceed with the search
						commonHighlyRatedProducts(custId1, custId2);
					}
					break;

				case 11: System.out.print("Enter Product ID: ");
				int prodID = input.nextInt();
				pm.displayCustomersWhoReviewedProduct(prodID, cdata);
				break;
				case 12:
					System.out.println("Returning to Main Menu...");
					break;

				default:
					System.out.println("Invalid choice! Please try again.");
				}
			} catch (InputMismatchException e) {
				System.out.println("Invalid input! Please try again");
				input.nextLine(); // Clear invalid input
			}
		} while (choice != 12);
	}

	// Helper method to find common highly rated products between two customers
	public static void commonHighlyRatedProducts(int custId1, int custId2) {
		System.out.println("\n=== Common Highly Rated Products (Rating > 4.0) ===");
		System.out.println("Between Customer " + custId1 + " and Customer " + custId2);
		System.out.println("------------------------------------------------------------------------------------------------");

		// Get all products
		LinkedList<Product> allProducts = pm.getAllProducts();
		if (allProducts.empty()) {
			System.out.println("No products available.");
			return;
		}

		boolean foundCommon = false;

		// Iterate through all products
		allProducts.findFirst();
		while (true) {
			Product product = allProducts.retrieve();
			double avgRating = product.getAverageRating();

			// Check if product has average rating > 4.0
			if (avgRating > 4.0) {
				// Check if both customers reviewed this product
				boolean cust1Reviewed = false;
				boolean cust2Reviewed = false;

				LinkedList<Review> productReviews = product.getReviews();
				if (!productReviews.empty()) {
					productReviews.findFirst();
					while (true) {
						Review review = productReviews.retrieve();
						if (review.getCustomerId() == custId1) {
							cust1Reviewed = true;
						}
						if (review.getCustomerId() == custId2) {
							cust2Reviewed = true;
						}
						if (cust1Reviewed && cust2Reviewed) break;
						if (productReviews.last()) break;
						productReviews.findNext();
					}
				}

				// If both customers reviewed this highly rated product, display it
				if (cust1Reviewed && cust2Reviewed) {
					if (!foundCommon) {
						System.out.printf("%-12s %-25s %-10s %-8s %-8s%n",
								"ProductID", "Name", "Price", "Stock", "AvgRating");
						System.out.println("------------------------------------------------------------------------------------------------");
						foundCommon = true;
					}
					product.display();
				}
			}

			if (allProducts.last()) break;
			allProducts.findNext();
		}

		if (!foundCommon) {
			System.out.println("No common highly rated products found between these customers.");
		}
		System.out.println("------------------------------------------------------------------------------------------------");
	}

	// =====================================================================================================================
	public static void CustomersMenu() {

		int choice;
		do {
			System.out.println("\n***** CUSTOMER MENU *****");
			System.out.println("1. Display all customers");
			System.out.println("2. Register new customer");
			System.out.println("3. Search for customer by ID");
			System.out.println("4. Place New Order for specific customer");//not 
			System.out.println("5. View Order history for specific customer");
			System.out.println("6. Display a list of all customers sorted alphabetically");
			System.out.println("7. Return Main menu");
			System.out.print("Enter your choice: ");

			try {
				choice = input.nextInt();

				switch (choice) {
				case 1: cdata.displayAllCustomers();
				break;
				case 2:
					System.out.print("Enter ID: ");
					int custId = input.nextInt();
					input.nextLine(); // Clear buffer
					System.out.print("Enter name: ");
					String name = input.nextLine();

					System.out.print("Enter email: ");
					String email = input.nextLine();

					while( !email.contains("@")||email.indexOf("@")==0||
							email.indexOf("@")==email.length() - 1||
							email.lastIndexOf(".")<=email.indexOf("@") + 1||
							email.lastIndexOf(".") == email.length() - 1||email.contains(" ")  ) 
					{
						System.out.print("Enter a valid email: ");
						email = input.nextLine();
					}
					cdata.addCustomer(new Customer(custId, name, email));
					break;
				case 3:			
					System.out.print("Enter Customer ID to search: ");
					int custid = input.nextInt();
					Customer foundById = cdata.searchById(custid);
					if (foundById != null) {
						System.out.println("\nCustomer Found:");
						System.out.printf("%-15s %-20s %-25s%n", "CustomerID", "Name" , "Email");
						System.out.println("------------------------------------------------------------------------------------------------");
						foundById.display();
					} 
					else {
						System.out.println("Customer with ID " + custid + " not found");
					}

					break;
				case 4: 
					placeOrder(); 
					break;

				case 5:
					System.out.print("Enter ID: ");
					int cId = input.nextInt();
					Customer customer = cdata.searchById(cId);
					if (customer == null) {
						System.out.println("Customer with ID " + cId + " not found");
					} else {
						customer.displayOrders();
					}
					break;
				case 6:
					cdata.displayCustomersSorted();
					System.out.println("------------------------------------------------------------------------------------------------");
					break;
				case 7:
					System.out.println("Returning to Main Menu...");
					break;

				default:
					System.out.println("Invalid choice! Please try again.");
				}
			} catch (InputMismatchException e) {
				System.out.println("Invalid input! Please try again");
				input.nextLine(); // Clear invalid input
				choice = 0; // Reset choice to stay in menu
			}
		} while (choice != 7);
	}




	// =====================================================================================================================
	public static void OrdersMenu() {
		int choice = 0;
		do {		
			System.out.println("\n**** ORDER MENU ****");
			System.out.println("1. Display all orders");
			System.out.println("2. Place New Order");
			System.out.println("3. Cancel Order");
			System.out.println("4. Update Order (Status)");
			System.out.println("5. Search By Order ID");
			System.out.println("6. All orders between two dates");
			System.out.println("7. Return Main menu");
			System.out.print("Enter your choice: ");

			try {
				choice = input.nextInt();

				switch (choice) {
				case 1: order.displayAllOrders();
				break;
				case 2:
					placeOrder();
					System.out.println("------------------------------------------------------------------------------------------------");
					break;

				case 3:
					cancelOrder();
					System.out.println("------------------------------------------------------------------------------------------------");
					break;

				case 4:
					System.out.println("update to new status...");
					if (order.getOrdersList().empty())
						System.out.println("empty Orders data");
					else {
						System.out.print("Enter order ID: ");
						int orderID = input.nextInt();
						input.nextLine(); // Clear buffer
						System.out.println("Enter new status (pending, shipped, delivered, cancelled):");
						String st = input.next();
						Order.Status s = Order.Status.valueOf(st.toLowerCase());
						order.UpdateOrderState(orderID, s);
						System.out.println("The order status has been updated successfully.");
						System.out.println("------------------------------------------------------------------------------------------------");
					}
					break;

				case 5:
					if (order.getOrdersList().empty())
						System.out.println("empty Orders data");
					else {
						System.out.print("Enter order ID: ");
						int orderID = input.nextInt();
						if (order.searchOrder(orderID) == null)
							System.out.println("order not found");
						else
						{System.out.printf("%-10s %-12s %-20s %-12s %-15s %-10s%n",
								"OrderID", "CustomerID", "ProductIDs", "TotalPrice", "Date", "Status");
						System.out.println("------------------------------------------------------------------------------------------------");
						order.searchOrder(orderID).display();}
					}
					System.out.println("------------------------------------------------------------------------------------------------");
					break;

				case 6:

					System.out.print("Enter first date (M/d/yyyy): ");
					String date1 = input.next();
					System.out.print("Enter second date (M/d/yyyy): ");
					String date2 = input.next();

					try {
						LinkedList<Order> dateOrders = order.BetweenTwoDates(date1, date2);

						if (dateOrders.empty())
							System.out.println("No orders found between " + date1 + " and " + date2);
						else {
							System.out.println("\n--- ORDERS BETWEEN " + date1 + " AND " + date2 + "---");
							System.out.printf("%-10s %-12s %-20s %-12s %-15s %-10s%n", "OrderID", "CustomerID",
									"ProductIDs", "TotalPrice", "Date", "Status");
							System.out.println("------------------------------------------------------------------------------------------------");

							dateOrders.findFirst();
							while (true) {
								dateOrders.retrieve().display();
								if (dateOrders.last())
									break;
								dateOrders.findNext();
							} // end while
							System.out.println("------------------------------------------------------------------------------------------------");
							System.out.println("Total orders found: " + dateOrders.size());
							System.out.println("------------------------------------------------------------------------------------------------");
						} // end else
					} catch (DateTimeParseException e) {
						System.out.println("Invalid date format! Please use M/d/yyyy (e.g., 11/14/2025)");
					} catch (Exception e) {
						System.out.println("Error: " + e.getMessage());
					}
					break;
				case 7:
					System.out.println("Returning to Main Menu...");
					break;

				default:
					System.out.println("Invalid choice! Try again");
				}
			} catch (InputMismatchException e) {
				System.out.println("Invalid input");
				input.nextLine(); // Clear invalid input
			} catch (IllegalArgumentException e) {
				System.out.println("Invalid input");
				input.nextLine(); // Clear invalid input
			}

		} while (choice != 7);
	}

	// =====================================================================================================================
	public static void ReviewsMenu() {
		int choice = 0;
		do {
			System.out.println("\n**** REVIEWS MENU ****");
			System.out.println("1. Display All Reviews");
			System.out.println("2. Add Review ");
			System.out.println("3. Edit Review (rating, comment) ");
			System.out.println("4. Get Average Rating for Product");
			System.out.println("5. Top 3 Products by Rating");
			System.out.println("6. Display Reviews by Customer");
			System.out.println("7. Return to Main Menu");
			System.out.print("Enter your choice: ");

			try {
				choice = input.nextInt();

				switch(choice) {
				case 1:
					rm.displayAllReviews();
					break;
				case 2: 
					System.out.print("Enter Review ID: "); 
					int reviewId = input.nextInt(); 
					System.out.print("Enter Product ID: "); 
					int productId = input.nextInt(); 
					System.out.print("Enter Customer ID: "); 
					int custId = input.nextInt(); 
					
					System.out.print("Enter Rating (1-5): "); 
					int rating = input.nextInt(); 
					while(rating<0||rating>5) {
					System.out.println("Invalid rating! ");//validating rating
					System.out.print("Enter Rating (1-5): "); 
					rating = input.nextInt(); }
		
					System.out.print("Enter Comment: "); 
					String comment = input.nextLine(); 

					Review newReview = new Review(reviewId, productId, custId, rating, comment); 
					rm.addReview(newReview); 
					break;

				case 3:
					System.out.print("Enter Review ID to edit: ");
					int editReviewId = input.nextInt();
					Review existingReview = rm.searchById(editReviewId);
					if (existingReview != null) {
						System.out.print("Enter new Rating (1-5): ");
						int newRating = input.nextInt();
						input.nextLine(); // Clear buffer
						System.out.print("Enter new Comment: ");
						String newComment = input.nextLine();

						Review updatedReview = new Review(editReviewId, existingReview.getProductId(), 
								existingReview.getCustomerId(), newRating, newComment);
						rm.updateReview(editReviewId, updatedReview);
					} else {
						System.out.println("Review not found!");
					}
					break;

				case 4:
					System.out.print("Enter Product ID: ");
					int avgProductId = input.nextInt();
					Product product = pm.searchById(avgProductId);
					if (product != null) {
						double avgRating = product.getAverageRating();
						System.out.printf("Average rating for product '%s': %.1f%n", 
								product.getName(), avgRating);
					} else {
						System.out.println("Product not found!");
					}
					break;

				case 5:
					System.out.println("\n--- TOP 3 RATED PRODUCTS ---");
					pm.top3ByRating();
					break;

				case 6:
					System.out.print("Enter Customer ID: ");
					int customerId = input.nextInt();
					rm.reviewsByCustomer(customerId);
					break;

				case 7:
					System.out.println("Returning to Main Menu...");
					break;

				default:
					System.out.println("Invalid choice! Please try again.");
				}
			} catch (InputMismatchException e) {
				System.out.println("Invalid input! Please try again");
				input.nextLine(); // Clear invalid input
			}
		} while (choice != 7);
	}


	// =====================================================================================================================
	public static void placeOrder() {

		Order new_order = new Order();
		double total_price = 0;

		// 1) reading order id and make sure no duplication occurs
		System.out.println("Enter order ID: ");
		int oid = input.nextInt();
		while (order.searchOrder(oid) != null) {//ا
			System.out.println("Order id is available , try again");
			oid = input.nextInt();
		}

		//2)reading cus id
		System.out.println("Enter customer ID:");
		int cid = input.nextInt();
		while (cdata.searchById(cid) == null) {
			System.out.println("Customer ID is not available , try again");
			cid = input.nextInt();
		}

		// 3)product 
		char answer = 'y';
		while (answer == 'y' || answer == 'Y') {

			System.out.println("Enter product ID:");
			int pid = input.nextInt();

			Product pp = pm.searchById(pid);

			if (pp == null) {
				System.out.println("No such product id");
			} else {
				if (pp.getStock() == 0) {
					System.out.println("product out of stock , try another time");
				} else {
					pp.setStock(pp.getStock() - 1);

					System.out.println("product added to order");

					new_order.addId(pp.getProductId());
					total_price += pp.getPrice();
				}
			}

			System.out.println("Do you want to continue adding product? (Y/N)");
			answer = input.next().charAt(0);
		}

		// zero products -> cancel the order
		if (total_price == 0) {
			System.out.println("Cannot complete order - no products added!");
			System.out.println("Order cancelled.");
			return;
		}

		// 4) validates the date
		LocalDate Ldate = null;
		boolean validDate = false;
		while (!validDate) {
			try {
				System.out.println("Enter order date (dd/mm/yyyy):");
				DateTimeFormatter dateformat = DateTimeFormatter.ofPattern("dd/MM/yyyy");
				Ldate = LocalDate.parse(input.next(), dateformat);

				LocalDate today = LocalDate.now();
				if (Ldate.isBefore(today)) {
					System.out.println("Error: Date cannot be in past!");
					System.out.println("Please enter today or future date");
				} else {
					validDate = true;
				}

			} catch (DateTimeParseException e) {
				System.out.println("Invalid date format!");
				input.nextLine(); // clear invalid input
			}
		}

		// 5)Status(enum)
		Order.Status s = null;
		boolean flagStatus = false;

		while (!flagStatus) {
			System.out.println("Enter new status (pending, shipped, delivered, cancelled):");
			String status = input.next().toLowerCase();
			try {
				s = Order.Status.valueOf(status);
				flagStatus = true;
			} catch (IllegalArgumentException e) {
				System.out.println("Invalid status! Try again.");
			}
		}

		// 6) all is good now full the info
		new_order.setOrderId(oid);
		new_order.setCustomerId(cid);
		new_order.setTotalPrice(total_price);
		new_order.setOrderDate(Ldate);
		new_order.setStatus(s);
		//7)add 
		order.addOrder(new_order);

		// 8) adding the order to the customer
		Customer cust = cdata.searchById(cid);   
		if (cust != null) {
			cust.addOrder(new_order);
		}

		System.out.println("Order had been added ");
		System.out.printf("%-10s %-12s %-20s %-12s %-15s %-10s%n",
				"OrderID", "CustomerID", "ProductIDs", "TotalPrice", "Date", "Status");
		System.out.println("------------------------------------------------------------------------------------------------");
		new_order.display();
	}
	// =====================================================================================================================
	public static void cancelOrder() {
		System.out.println("Enter order ID to cancel: ");
		int oid = input.nextInt();

		// 1) search 
		Order cancel_order = order.searchOrder(oid);
		while (cancel_order == null) {
			System.out.println("order id is not available , try again");
			oid = input.nextInt();
			cancel_order = order.searchOrder(oid);
		}

		// 2) deleting
		order.removeOrder(oid);

		// 3) find the customer
		int cid = cancel_order.getCustomerId();
		Customer cust = cdata.searchById(cid);
		if (cust == null) {
			System.out.println("Customer not found!");
			return;
		}

		//4)removes the order from customer order list
		LinkedList<Order> custOrders = cust.getOrders();
		if (!custOrders.empty()) {
			custOrders.findFirst();
			while (true) {
				if (custOrders.retrieve().getOrderId() == oid) {
					custOrders.remove();
					break;
				}
				if (custOrders.last()) break;
				custOrders.findNext();
			}
		}

		System.out.println("Order " + oid + " has been cancelled successfully");
	}
	public static void linkOrdersToCustomers() {
		//all oeders
		LinkedList<Order> allOrders = order.getOrdersList();
		if (allOrders.empty()) return;

		allOrders.findFirst();
		while (true) {
			Order o = allOrders.retrieve();
			if (o != null) {
				int cid = o.getCustomerId();
				Customer cust = cdata.searchById(cid);
				if (cust != null) {
					cust.addOrder(o);  
				}
			}
			if (allOrders.last()) break;
			allOrders.findNext();
		}
	}

	//==================================================================================================================================================
	// Main:
	public static void main(String[] args) {
		boolean dataloaded = loadData();
		int choice;
		if (dataloaded) {
			do {
				choice = mainMenu();
				switch (choice) {
				case 1:
					productsMenu();
					break;
				case 2:
					CustomersMenu();
					break;
				case 3:
					OrdersMenu();
					break;
				case 4:
					ReviewsMenu();
					break;
				case 5:
					System.out.println("Exiting...");
					break;
				default:
					System.out.println("invalid choice! try again");
				}// end of switch
			} while (choice != 5);// end of do while
		} // end of if
		else
			System.out.println("Cannot proceed unless all files are loaded successfully");
	}// end of main

}
